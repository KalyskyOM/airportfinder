 ...
 * aviation weather (METAR/TAF) and NOTAM data.
 * 
...
public class AvwxApiService {
    ...
    /**
     * AsyncTask to fetch NOTAM data from AVWX API
     */
    private class FetchNotamTask extends AsyncTask<String, Void, List<NotamData>> {
        ...
        protected List<NotamData> doInBackground(String... params) {
            ...
            try {
                ...
                if (responseCode == HttpURLConnection.HTTP_OK) {
                    ...
                    List<NotamData> result = parseNotamResponse(response.toString());
                    
                    // Cache the result
                    if (result != null) {
                        cache.cacheNotams(icaoCode, result);
                    }
        ...
        /**
         * Parse the JSON response from the AVWX API into a list of NotamData objects
         */
        private List<NotamData> parseNotamResponse(String jsonResponse) throws JSONException {
            ...
            if (json.has("data") && !json.isNull("data")) {
                ...
                for (int i = 0; i < notams.length(); i++) {
                    ...
                    if (notamJson.has("start_time") && !notamJson.isNull("start_time")) {
                        notam.setStartTime(notamJson.getString("start_time"));
                    }
                    
                    if (notamJson.has("end_time") && !notamJson.isNull("end_time")) {
                        notam.setEndTime(notamJson.getString("end_time"));
                    }
